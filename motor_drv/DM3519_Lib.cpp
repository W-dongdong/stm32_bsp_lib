...
DM3519::DM3519(CANDevice* can, uint16_t ID)
    : m_CAN(can)
    , m_ID(ID)
...
uint8_t DM3519::ParseFeedback(CanMsg* RxMsg)
{
    ...
    else
    {
        ...
        m_Pos     = (RxMsg->Data[1] << 8) | RxMsg->Data[2];
        m_Vel     = (int16_t)(((RxMsg->Data[3] << 4) | ((RxMsg->Data[4] >> 4) & 0x0F)) - 0x7FF);
        m_Torque  = ((RxMsg->Data[4] & 0xF) << 8) |  RxMsg->Data[5];
